...
from addons.account.report.common_report_header import common_report_header

class aged_trial_report_detailed(report_sxw.rml_parse, common_report_header):

    def __init__(self, cr, uid, name, context):
        super(aged_trial_report_detailed, self).__init__(cr, uid, name, context=context)
        self.total_account = []
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        return super(aged_trial_report_detailed, self).set_context(objects, data, ids, report_type=report_type)
    ...
    def _get_lines(self, form):
        ...
        for partner in partners:
            ...
            # Retrieving the Payment term of the Partner to display in the Detailed report
            # Establish a connection object with ir.property table.
            # Establishing the Connection with Account payment term to display the name...


            filter_ids = partner_obj.search(self.cr, self.uid, [('res_id','=',"res.partner,%s"%partner['id']),('name','=','property_payment_term')])
            payment_term_ids = partner_obj.browse(self.cr,self.uid,filter_ids)
            if len(payment_term_ids) > 0:
               # print "Filter Ids Here..............",payment_term_ids
                payment_term=payment_term_ids[0].value_reference
            ...
            inlines=self._display_screen(partner['id'],form,current_records)
            ...
            for data in reorder_values:
                    t_dict.update({'0':t_dict['0']+data['0'],
                                   '1':t_dict['1']+data['1'],
                                   '2':t_dict['2']+data['2'],
                                   '3':t_dict['3']+data['3'],
                                   '4':t_dict['4']+data['4'],
                                   'due':t_dict['due']+data['due']})
    ...
    # Calculating the Partial Balance in this Function...................
    def _display_screen(self,ids,form,detailed):
        """
        Returns a dict that contains new values and context

        @param ids: latest value from user input for field partner_id
        @param form: other arguments
        @param detailed: Lines with All the Values of particular id
...
report_sxw.report_sxw('report.r3x_aged_partner_balance.aged_trial_report_detailed', 
                      'res.partner',
                      'addons/r3x_aged_partner_balance/report/account_aged_partner_balance_detailed.rml',
                      parser=aged_trial_report_detailed, 
                      header="internal landscape")


# Prinitng the Aged Partner Balance Period Length without Detail Lines..............
...
report_sxw.report_sxw('report.r3x_aged_partner_balance.aged_trial_balance_detail_without_details', 
                      'res.partner',
                      'addons/r3x_aged_partner_balance/report/account_aged_partner_balance.rml',
